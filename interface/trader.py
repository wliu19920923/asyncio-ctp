...
class TraderApi(thosttraderapi.CThostFtdcTraderSpi):
    ...
    def get_balance(self):
        self.api.ReqQryTradingAccount(fields.QryTradingAccountField(self.broker_id, self.investor_id).field, self.request_id)

    def get_position(self, instrument_id):
        self.api.ReqQryInvestorPosition(fields.QryInvestorPositionField(self.broker_id, self.investor_id, instrument_id), self.request_id)

    def create_order(self, exchange_id, instrument_id, offset_flag, direction, price, volume):
        self.api.ReqOrderInsert(fields.OrderInsertField(self.broker_id, self.investor_id, exchange_id, instrument_id, offset_flag, direction, price, volume), self.request_id)

    def get_settlement_info(self, trading_day):
        self.api.ReqQrySettlementInfo(fields.QrySettlementInfoField(self.broker_id, self.investor_id, trading_day), self.request_id)

    def confirm_settlement_info(self):
        self.api.ReqSettlementInfoConfirm(fields.SettlementInfoConfirmField(self.broker_id, self.investor_id), self.request_id)

    def OnFrontConnected(self):
        self.api.ReqAuthenticate(fields.AuthenticateField(self.app_id, self.auth_code, self.broker_id, self.investor_id), self.request_id)
    ...
    def OnRspAuthenticate(self, pRspAuthenticateField: thosttraderapi.CThostFtdcRspAuthenticateField, pRspInfo: thosttraderapi.CThostFtdcRspInfoField, nRequestId: int, bIsLast: bool):
        ...
        self.api.ReqUserLogin(fields.UserLoginField(self.broker_id, self.investor_id, self.password), self.request_id)
